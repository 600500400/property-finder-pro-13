...
export async function computeScraperHealth(): Promise<SourceHealth[]> {
  ...
  for (const [source, list] of bySource.entries()) {
    ...
    // Health is based on the OUTCOME of the latest completed run, not elapsed
    // time. Sources scrape at different cadences (some every 8–12 h), so any
    // fixed time window misfires. Rule:
    //   - no completed runs in 7 d                       → red
    //   - latest completed = error                       → red
    //   - latest completed = success, items < 50% base   → red
    //   - latest completed = success, items 50–70% base  → amber (advisory)
    //   - latest completed = success, items ≥ 70% base   → green
    if (!last) {
      reasons.push("Žádný dokončený běh v posledních 7 dnech");
      indicator = "red";
    } else if (last.status === "error") {
      reasons.push(`Poslední dokončený běh selhal: ${last.error_message ?? "neznámá chyba"}`);
      indicator = "red";
      ...
      if (baseline !== null && baseline > 0) {
        ...
        if (dropPct >= 50) {
          reasons.push(
            `Pokles o ${dropPct.toFixed(0)}% vs baseline (${last.items_found} vs ${baseline})`,
          );
          indicator = "red";
        } else if (dropPct > 30) {
